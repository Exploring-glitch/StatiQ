  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-14">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {agents.map((a) => (
          <div key={a.name} className="rounded-xl border border-white/10 bg-surface p-5">
            <p className="text-sm font-bold text-white">{a.name}</p>
            <p className="mt-1 text-xs text-emerald-400">● 3 agents working · Live</p>
            <div className="mt-3 grid grid-cols-2 gap-2 text-center text-xs">
              <div className="rounded-md bg-card p-2"><p className="font-bold text-white">{a.evaluated}</p><p className="text-slate-500">evaluated</p></div>
              <div className="rounded-md bg-card p-2"><p className="font-bold text-white">{a.matched}</p><p className="text-slate-500">matched</p></div>
              <div className="rounded-md bg-card p-2"><p className="font-bold text-white">{a.pitched}</p><p className="text-slate-500">pitched</p></div>
              <div className="rounded-md bg-card p-2"><p className="font-bold text-white">{a.replied}</p><p className="text-slate-500">replied</p></div>
            </div>
          </div>
        ))}
      </div>
