  ...
  return (
    <section id="companies" className="mx-auto max-w-6xl px-4 py-14">
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {items.map((it) => (
          <div key={it.n} className="rounded-xl border border-white/10 bg-panel p-6">
            <p className="text-xs font-bold text-neutral-500">{it.n}</p>
            <h3 className="mt-1 text-lg font-bold text-white">{it.title}</h3>
            <p className="mt-1 text-sm font-medium text-neutral-300">{it.sub}</p>
            <p className="mt-2 text-sm text-neutral-400">{it.text}</p>
          </div>
        ))}
      </div>
      <div className="mt-6 flex flex-wrap gap-3">
        <Link to="/for-companies" className="rounded-md bg-[#f4f4f5] px-4 py-2 text-sm font-semibold text-black hover:bg-neutral-300">
          Find your next hire →
        </Link>
