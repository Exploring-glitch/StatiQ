...
export default function PostJobPage() {
  ...
  const input = 'w-full rounded-md border border-neutral-200 bg-white px-3 py-2 text-sm text-neutral-900 placeholder:text-neutral-400';

  return (
    <section className="mx-auto max-w-6xl px-4 py-10">
      <p className="text-xs font-semibold uppercase tracking-wide text-accent">Employer studio</p>
      <h1 className="mt-2 text-3xl font-bold text-neutral-900">Post a job — free.</h1>
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <form onSubmit={submit} className="space-y-3 rounded-xl border border-neutral-200 bg-[#F4F4F2] p-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="flex items-center gap-2 rounded-md border border-neutral-200 px-3 py-2 text-sm text-neutral-600">
              <input type="checkbox" checked={form.remote} onChange={set('remote')} className="accent-red-600" /> Remote
            </label>
          </div>
        ...
        <div className="h-fit rounded-xl border border-neutral-200 bg-white p-6">
          <h2 className="text-sm font-bold text-neutral-900">Your posted jobs</h2>
          {mine.length === 0 ? (
            <p className="mt-2 text-xs text-neutral-400">Nothing posted yet — your jobs will appear here and on the Jobs page.</p>
          ) : (
            <ul className="mt-3 space-y-2">
              {mine.map((j) => (
                <li key={j._id || j.id} className="flex items-center justify-between rounded-md border border-neutral-200 p-2 text-sm">
                  <span className="text-neutral-700">{j.title} <span className="text-xs text-neutral-400">· {j.location}</span></span>
