    ...
    <section className="mx-auto max-w-5xl px-4 py-10">
      <div className="grid overflow-hidden rounded-2xl border border-white/10 bg-panel shadow-2xl shadow-black/40 md:grid-cols-2">
        ...
        <div className="relative hidden flex-col justify-between overflow-hidden bg-gradient-to-br from-emerald-400/20 via-panel to-panel p-8 md:flex">
          ...
          <div className="relative">
            ...
            <p className="mt-2 text-sm leading-relaxed text-neutral-400">
              Log in to post jobs free, review applicants, and manage your pipeline in one place.
            </p>
            <ul className="mt-6 space-y-2.5 text-sm text-neutral-300">
              {['Unlimited free job posts + ATS', '10M+ startup-minded candidates', '5–10 qualified candidates / week'].map((t) => (
          ...
          <div className="relative mt-8 grid grid-cols-2 gap-3">
            {[
              ...
              <div key={l} className="rounded-xl border border-white/10 bg-ink/50 p-3 text-center">
        ...
        <div className="p-6 sm:p-8">
          ...
          <p className="mt-1 text-sm text-neutral-400">Log in to your hiring workspace to continue.</p>

          <form className="mt-6 space-y-4" onSubmit={submit}>
            <div>
              <label htmlFor="employer-email" className={labelCls}>
                Work email
              </label>
              <input
                id="employer-email"
            ...
            <div>
              <label htmlFor="employer-password" className={labelCls}>
                Password
              </label>
              <div className="relative">
                <input
          ...
          <p className="text-center text-xs leading-relaxed text-neutral-400">
            No employer account?{' '}
            <Link to="/signup/hire" className="font-semibold text-accent hover:underline">
              Create one
            </Link>
            <br />
            <span className="text-neutral-500">
              Looking for a job?{' '}
              <Link to={`/login/job${next !== '/dashboard' ? `?next=${encodeURIComponent(next)}` : ''}`} className="text-accent hover:underline">
                Seeker log in
              </Link>
            </span>
          </p>
