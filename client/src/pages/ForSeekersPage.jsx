...
export default function ForSeekersPage() {
  return (
    <section className="mx-auto max-w-6xl px-4 py-10">
      ...
      <h1 className="mt-2 max-w-2xl text-3xl font-bold text-neutral-900">Find work that matters.</h1>
      <p className="mt-2 max-w-2xl text-sm text-neutral-500">
        Join 10M+ candidates who want startup roles. Free forever for seekers.
      </p>
      <div className="mt-4 flex gap-3">
        <Link to="/signup/job" className="rounded-md bg-neutral-900 px-4 py-2 text-sm font-semibold text-white hover:bg-neutral-700">
          Sign up for free →
        </Link>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        {steps.map((s) => (
          <div key={s.n} className="rounded-xl border border-neutral-200 bg-[#F4F4F2] p-6">
            <p className="text-xs font-bold text-accent">{s.n}</p>
            <p className="mt-1 text-sm font-bold text-neutral-900">{s.t}</p>
            <p className="mt-1 text-sm text-neutral-500">{s.d}</p>
          </div>
        ))}
      </div>
      <h2 className="mt-10 text-xl font-bold text-neutral-900">Featured roles</h2>
