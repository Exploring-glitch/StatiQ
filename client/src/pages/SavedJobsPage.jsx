  ...
  return (
    <section className="mx-auto max-w-4xl px-4 py-10">
      ...
      <div className="mt-6 space-y-3">
        {items.map((j) => (
          <div key={j.id} className="flex items-center justify-between gap-3 rounded-xl border border-white/10 bg-surface p-4">
            <div>
              <p className="text-sm font-bold text-white">{j.role}</p>
              <p className="text-xs text-slate-400">{j.company} · {j.location} · {j.salary}</p>
            </div>
            <div className="flex gap-2">
              <Link to={`/jobs/${j.id}`} className="rounded-md bg-white px-3 py-1 text-xs font-semibold text-base">View</Link>
              <button onClick={() => unsave(j.id)} className="rounded-md border border-white/15 px-3 py-1 text-xs text-white">Remove</button>
            </div>
          </div>
        ))}
      </div>
